...
async function createIndexFile(iconsType = "mi", format = "cjs") {
  ...
  const allFiles = fs.readdirSync(
    path.resolve(__dirname, targetDirectory, format)
  );
  const indexFileContents = [];

  console.log(
    chalk.green("Generating index reexports for ", iconsType, " ") +
      chalk.yellow(format.toUpperCase())
  );
  ...
  if (format === "cjs") {
    fs.writeFileSync(
      path.resolve(__dirname, targetDirectory, "index.js"),
      indexFileContents.join("\n")
    );
  } else {
    fs.writeFileSync(
      path.resolve(__dirname, targetDirectory, "index.esm.js"),
      indexFileContents.join("\n")
    );
  }

  fs.writeFileSync(
    path.resolve(__dirname, targetDirectory, format, "utils.js"),
    handleDefaultPropsFileContent[format]
  );
